...
public class Individual extends Habitacion{
    ...
    public Individual(String codigoHotel, double precio, int capacidad, int numerodeHabitacion, Estado estado) {
     ...
     public void tipoHabitacion(ArrayList<String[]> habitacionString, Hotel hotElegido, ArrayList<Habitacion> habitaciones, long dias) {
        ...
        for (int i = 0; i < habitacionString.size(); i++) {
            ...
            if (habitacionString.get(i)[0].equals(hotElegido.getCodigoHotel()) && habitacionString.get(i)[5].equals("DISPONIBLE")) {
                Habitacion habi = new Individual(habitacionString.get(i)[0], Double.valueOf(habitacionString.get(i)[2]), Integer.parseInt(habitacionString.get(i)[3]), Integer.parseInt(habitacionString.get(i)[4]), Estado.valueOf(habitacionString.get(i)[5]));
                habitaciones.add(habi);
                int numero = (int) (Math.random() * habitaciones.size() + 1);
                System.out.println("Usted ha elegido una habitación Individual para un total de " + dias + " noche(s).");
